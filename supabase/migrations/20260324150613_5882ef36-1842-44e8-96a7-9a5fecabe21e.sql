...
CREATE OR REPLACE FUNCTION public.execute_btc_swap(
  p_token_id uuid, p_wallet_address text, p_trade_type text, p_amount numeric
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_confirmed_deposits numeric;
BEGIN
  -- 1. Lock the token row exclusively
  SELECT * INTO v_token
    FROM btc_meme_tokens WHERE id = p_token_id FOR UPDATE;

  IF NOT FOUND THEN RAISE EXCEPTION 'Token not found'; END IF;
  IF v_token.status = 'pending_genesis' THEN RAISE EXCEPTION 'Token genesis not yet confirmed'; END IF;
  IF v_token.status IN ('graduated', 'migrating', 'migration_blocked') THEN RAISE EXCEPTION 'Token has graduated, trading closed'; END IF;
  IF v_token.status != 'active' THEN RAISE EXCEPTION 'Token is not active for trading'; END IF;

  -- 2. CRITICAL: Verify user has REAL confirmed on-chain deposits
  SELECT COALESCE(SUM(amount_btc), 0) INTO v_confirmed_deposits
    FROM btc_deposit_ledger
    WHERE wallet_address = p_wallet_address AND confirmed = true;

  IF v_confirmed_deposits <= 0 THEN
    RAISE EXCEPTION 'No confirmed BTC deposit found. Deposit BTC first and wait for network confirmation.';
  END IF;
  ...
  IF p_trade_type = 'buy' THEN
    ...
    SELECT balance_btc, total_deposited INTO v_bal_btc, v_total_deposited
      ...
      WHERE wallet_address = p_wallet_address FOR UPDATE;

    IF NOT FOUND OR v_bal_btc < v_btc_amount THEN
      RAISE EXCEPTION 'Insufficient BTC balance (available: %)', COALESCE(v_bal_btc, 0);
    END IF;
    ...
    SELECT id, balance, total_bought, total_sold, avg_buy_price_btc
      ...
      WHERE token_id = p_token_id AND wallet_address = p_wallet_address FOR UPDATE;
    ...
    IF FOUND THEN
      ...
      UPDATE btc_meme_balances SET
        ...
        avg_buy_price_btc = v_new_avg, updated_at = now()
    ...
    v_avg_buy_price := NULL; v_pnl_btc := NULL; v_pnl_percent := NULL;

  ELSIF p_trade_type = 'sell' THEN
    v_token_amount := p_amount;

    SELECT id, balance, total_sold, avg_buy_price_btc
      INTO v_token_bal_id, v_token_bal, v_token_total_sold, v_token_avg_price
      FROM btc_meme_balances
      WHERE token_id = p_token_id AND wallet_address = p_wallet_address FOR UPDATE;
    ...
    -- Compute PnL
    v_avg_buy_price := COALESCE(v_token_avg_price, 0);
    v_exec_price := CASE WHEN v_new_virtual_tokens > 0 THEN v_new_virtual_btc / v_new_virtual_tokens ELSE 0 END;
    IF v_avg_buy_price > 0 THEN
      v_pnl_btc := (v_exec_price - v_avg_buy_price) * v_token_amount;
      v_pnl_percent := ((v_exec_price - v_avg_buy_price) / v_avg_buy_price) * 100;
    ELSE
      v_pnl_btc := NULL; v_pnl_percent := NULL;
    END IF;

    -- Deduct tokens
    UPDATE btc_meme_balances SET
      balance = balance - v_token_amount,
      total_sold = COALESCE(total_sold, 0) + v_token_amount, updated_at = now()
    WHERE id = v_token_bal_id;

    -- Credit BTC to user
    SELECT balance_btc INTO v_user_btc_bal
      FROM btc_trading_balances WHERE wallet_address = p_wallet_address FOR UPDATE;
  ...
  -- Count holders (ignore dust < 1 token)
  SELECT count(*) INTO v_holder_count
    FROM btc_meme_balances WHERE token_id = p_token_id AND balance >= 1;
  ...
  UPDATE btc_meme_tokens SET
    ...
    bonding_progress = v_new_progress, holder_count = v_holder_count,
    trade_count = trade_count + 1, volume_btc = volume_btc + v_btc_amount,
