...
-- Update execute_btc_swap: count holders only when balance >= 1 (ignore dust)
-- This is a targeted replacement of the holder count logic in the function
...
BEGIN
  ...
  -- Count holders: only balances >= 1 token (ignore dust from rounding)
  SELECT count(*) INTO v_holder_count
    FROM btc_meme_balances
    WHERE token_id = p_token_id AND balance >= 1;
  ...
  RETURN jsonb_build_object(
    ...
    'avgBuyPrice', v_avg_buy_price,
    'pnlBtc', v_pnl_btc,
    'pnlPercent', v_pnl_percent
  );
END;
$function$;
